...
resource "aws_security_group" "mytest_webservers"{
  tags = {
    Name = "${var.ENVIRONMENT}-mytest-webservers"
  }
...
resource "aws_launch_configuration" "launch_config_webserver" {
  ...
  security_groups = [aws_security_group.mytest_webservers.id]
...
resource "aws_autoscaling_group" "mytest_webserver" {
  name                      = "mytest_WebServers"
  max_size                  = 2
  min_size                  = 1
...
resource "aws_lb" "mytest-load-balancer" {
  name               = "${var.ENVIRONMENT}-mytest-lb"
  internal           = false
  load_balancer_type = "application"
  security_groups    = [aws_security_group.mytest_webservers_alb.id]
...
resource "aws_lb_listener" "webserver_listner" {
  load_balancer_arn = aws_lb.mytest-load-balancer.arn
  port              = "80"
  protocol          = "HTTP"
...
output "load_balancer_output" {
  value = aws_lb.mytest-load-balancer.dns_name
}
